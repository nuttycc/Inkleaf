...
Use `POST https://api.exa.ai/contents` when the agent already knows the URLs and needs clean,
LLM-ready extraction without running a new search. Start with one content mode: `highlights` for
compact agent context, `text` for broad page context, or `summary` for Exa-side compression.
...
Prefer `highlights: true` for the highest-quality default. Only use object form when the agent needs
a custom focus or budget.
...
Start with `subpages` around `5` to `10`, then increase only when the caller needs broader site
coverage.

## Response Fields and Statuses

Inspect `statuses` even when the HTTP status is 200. The endpoint can succeed for one URL and fail
for another in the same request.
...
Common per-URL error tags include `CRAWL_NOT_FOUND`, `CRAWL_TIMEOUT`, `CRAWL_LIVECRAWL_TIMEOUT`,
`SOURCE_NOT_AVAILABLE`, `UNSUPPORTED_URL`, and `CRAWL_UNKNOWN_ERROR`.
...
- Do not use `useAutoprompt`, `numSentences`, `highlightsPerUrl`, or older `livecrawl` string values
  in new requests.
...
- Pick one of `highlights`, `text`, or `summary` by default. Stack modes only when the caller truly
  needs multiple views of each page.
