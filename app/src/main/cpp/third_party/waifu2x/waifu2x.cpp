...
#endif
{
    ...
    if (vkdev)
    {
...

        {
            ...
            if (spirv.empty() ||
                waifu2x_preproc->create(spirv.data(), spirv.size() * 4, specializations) != 0)
                return -4;
        }

        {
            ...
            if (spirv.empty() ||
                waifu2x_postproc->create(spirv.data(), spirv.size() * 4, specializations) != 0)
                return -4;
        }
    }
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (tta_mode)
            {
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    int ret = ex.input("Input1", in_tile_gpu[ti]);
                    if (ret == 0)
                        ret = ex.extract("Eltwise4", out_tile_gpu[ti], cmd);
            ...
            else
            {
                ...
                ncnn::VkMat out_tile_gpu;
                {
                    ...
                    int ret = ex.input("Input1", in_tile_gpu);
                    if (ret == 0)
                        ret = ex.extract("Eltwise4", out_tile_gpu, cmd);
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (tta_mode)
            {
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    int ret = ex.input("Input1", in_tile[ti]);
                    if (ret == 0)
                        ret = ex.extract("Eltwise4", out_tile[ti]);
                    if (ret != 0)
                        return ret;
            ...
            else
            {
                ...
                ncnn::Mat out_tile;
                {
                    ...
                    int ret = ex.input("Input1", in_tile);
                    if (ret == 0)
                        ret = ex.extract("Eltwise4", out_tile);
                    if (ret != 0)
                        return ret;
                    if (out_tile.empty())
                        return ERROR_EMPTY_MAT;
