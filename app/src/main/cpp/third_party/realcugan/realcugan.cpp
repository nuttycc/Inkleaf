...
constexpr int ERROR_EMPTY_MAT = -100;

void record_first_error(std::atomic<int>& result, int error)
{
    int expected = 0;
    result.compare_exchange_strong(expected, error);
}
...
#endif
{
    ...
    if (vkdev)
    {
...

        {
            ...
            if (spirv.empty() ||
                realcugan_preproc->create(spirv.data(), spirv.size() * 4, specializations) != 0)
                return -4;
        }

        {
            ...
            if (spirv.empty() ||
                realcugan_postproc->create(spirv.data(), spirv.size() * 4, specializations) != 0)
                return -4;
        }

        {
            ...
            if (spirv.empty() ||
                realcugan_4x_postproc->create(spirv.data(), spirv.size() * 4, specializations) != 0)
                return -4;
        }
    }
    ...
    }
    {
        ...
        if (bicubic_3x->create_pipeline(net.opt) != 0)
            return -5;
    }
    {
        ...
        if (bicubic_4x->create_pipeline(net.opt) != 0)
            return -5;
    }
...
int RealCUGAN::process(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    std::atomic<int> process_result(0);

    //#pragma omp parallel for num_threads(2)
    for (int yi = 0; yi < ytiles; yi++)
    {
        if (process_result.load() != 0)
            continue;
        ...
        ncnn::VkMat in_gpu;
        {
            if (in.empty())
            {
                record_first_error(process_result, ERROR_EMPTY_MAT);
                continue;
            }
            ...
            if (process_result.load() == 0 && xtiles > 1)
            {
                const int ret = cmd.submit_and_wait();
                if (ret != 0)
                    record_first_error(process_result, ret);
                cmd.reset();
            }
        }
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (tta_mode)
            {
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    int ret = ex.input("in0", in_tile_gpu[ti]);
                    if (ret == 0)
                        ret = ex.extract("out0", out_tile_gpu[ti], cmd);
                    if (ret == 0 && out_tile_gpu[ti].empty())
                        ret = ERROR_EMPTY_MAT;
                    if (ret != 0)
                    {
                        record_first_error(process_result, ret);
                ...
                if (channels == 4)
                {
                    ...
                    if (scale == 2)
                    {
                        const int ret = bicubic_2x->forward(in_alpha_tile_gpu, out_alpha_tile_gpu, cmd, opt);
                        if (ret != 0 || out_alpha_tile_gpu.empty())
                        {
                            record_first_error(process_result, ret != 0 ? ret : ERROR_EMPTY_MAT);
                            break;
                        }
                    }
                    if (scale == 3)
                    {
                        const int ret = bicubic_3x->forward(in_alpha_tile_gpu, out_alpha_tile_gpu, cmd, opt);
                        if (ret != 0 || out_alpha_tile_gpu.empty())
                        {
                            record_first_error(process_result, ret != 0 ? ret : ERROR_EMPTY_MAT);
                            break;
                    ...
                    if (scale == 4)
                    {
                        const int ret = bicubic_4x->forward(in_alpha_tile_gpu, out_alpha_tile_gpu, cmd, opt);
                        if (ret != 0 || out_alpha_tile_gpu.empty())
                        {
                            record_first_error(process_result, ret != 0 ? ret : ERROR_EMPTY_MAT);
                            break;
            ...
            else
            {
                ...
                ncnn::VkMat in_alpha_tile_gpu;
                {
                    ...
                    if (in_tile_gpu.empty() || (channels == 4 && in_alpha_tile_gpu.empty()))
                    {
                        record_first_error(process_result, ERROR_EMPTY_MAT);
                        break;
                    }
                ...
                ncnn::VkMat out_tile_gpu;
                {
                    ...
                    int ret = ex.input("in0", in_tile_gpu);
                    if (ret == 0)
                        ret = ex.extract("out0", out_tile_gpu, cmd);
                    if (ret == 0 && out_tile_gpu.empty())
                        ret = ERROR_EMPTY_MAT;
                    if (ret != 0)
                    {
                        record_first_error(process_result, ret);
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (tta_mode)
            {
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    int ret = ex.input("in0", in_tile[ti]);
                    if (ret == 0)
                        ret = ex.extract("out0", out_tile[ti]);
                    if (ret != 0)
                        return ret;
                    if (out_tile[ti].empty())
                        return ERROR_EMPTY_MAT;
                }
                ...
                if (channels == 4)
                {
                    ...
                    if (scale == 2)
                    {
                        const int ret = bicubic_2x->forward(in_alpha_tile, out_alpha_tile, opt);
                        if (ret != 0)
                            return ret;
                        if (out_alpha_tile.empty())
                            return ERROR_EMPTY_MAT;
                    }
                    if (scale == 3)
                    {
                        const int ret = bicubic_3x->forward(in_alpha_tile, out_alpha_tile, opt);
                        if (ret != 0)
                            return ret;
                        if (out_alpha_tile.empty())
                            return ERROR_EMPTY_MAT;
                    ...
                    if (scale == 4)
                    {
                        const int ret = bicubic_4x->forward(in_alpha_tile, out_alpha_tile, opt);
                        if (ret != 0)
                            return ret;
                        if (out_alpha_tile.empty())
                            return ERROR_EMPTY_MAT;
            ...
            else
            {
                ...
                ncnn::Mat out_tile;
                {
                    ...
                    int ret = ex.input("in0", in_tile);
                    if (ret == 0)
                        ret = ex.extract("out0", out_tile);
                    if (ret != 0)
                        return ret;
                    if (out_tile.empty())
                        return ERROR_EMPTY_MAT;
...
int RealCUGAN::process_se(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    ret = process_se_stage0(inimage, in0, out0, opt, cache);

    std::vector<std::string> gap0 = {"gap0"};
    if (ret == 0)
        ret = process_se_sync_gap(inimage, gap0, opt, cache);

    std::vector<std::string> in1 = {"gap0"};
    std::vector<std::string> out1 = {"gap1"};
    if (ret == 0)
        ret = process_se_stage0(inimage, in1, out1, opt, cache);

    std::vector<std::string> gap1 = {"gap1"};
    if (ret == 0)
        ret = process_se_sync_gap(inimage, gap1, opt, cache);

    std::vector<std::string> in2 = {"gap0", "gap1"};
    std::vector<std::string> out2 = {"gap2"};
    if (ret == 0)
        ret = process_se_stage0(inimage, in2, out2, opt, cache);

    std::vector<std::string> gap2 = {"gap2"};
    if (ret == 0)
        ret = process_se_sync_gap(inimage, gap2, opt, cache);

    std::vector<std::string> in3 = {"gap0", "gap1", "gap2"};
    std::vector<std::string> out3 = {"gap3"};
    if (ret == 0)
        ret = process_se_stage0(inimage, in3, out3, opt, cache);

    std::vector<std::string> gap3 = {"gap3"};
    if (ret == 0)
        ret = process_se_sync_gap(inimage, gap3, opt, cache);

    std::vector<std::string> in4 = {"gap0", "gap1", "gap2", "gap3"};
    if (ret == 0)
        ret = process_se_stage2(inimage, in4, outimage, opt, cache);
...
int RealCUGAN::process_se_very_rough(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    std::vector<std::string> in0 = {};
    std::vector<std::string> out0 = {"gap0", "gap1", "gap2", "gap3"};
    ret = process_se_very_rough_stage0(inimage, in0, out0, opt, cache);

    std::vector<std::string> gap0 = {"gap0", "gap1", "gap2", "gap3"};
    if (ret == 0)
        ret = process_se_very_rough_sync_gap(inimage, gap0, opt, cache);
...
int RealCUGAN::process_cpu_se(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    std::vector<std::string> in0 = {};
    std::vector<std::string> out0 = {"gap0"};
    ret = process_cpu_se_stage0(inimage, in0, out0, cache);

    std::vector<std::string> gap0 = {"gap0"};
    if (ret == 0)
        ret = process_cpu_se_sync_gap(inimage, gap0, cache);

    std::vector<std::string> in1 = {"gap0"};
    std::vector<std::string> out1 = {"gap1"};
    if (ret == 0)
        ret = process_cpu_se_stage0(inimage, in1, out1, cache);

    std::vector<std::string> gap1 = {"gap1"};
    if (ret == 0)
        ret = process_cpu_se_sync_gap(inimage, gap1, cache);

    std::vector<std::string> in2 = {"gap0", "gap1"};
    std::vector<std::string> out2 = {"gap2"};
    if (ret == 0)
        ret = process_cpu_se_stage0(inimage, in2, out2, cache);

    std::vector<std::string> gap2 = {"gap2"};
    if (ret == 0)
        ret = process_cpu_se_sync_gap(inimage, gap2, cache);

    std::vector<std::string> in3 = {"gap0", "gap1", "gap2"};
    std::vector<std::string> out3 = {"gap3"};
    if (ret == 0)
        ret = process_cpu_se_stage0(inimage, in3, out3, cache);

    std::vector<std::string> gap3 = {"gap3"};
    if (ret == 0)
        ret = process_cpu_se_sync_gap(inimage, gap3, cache);

    std::vector<std::string> in4 = {"gap0", "gap1", "gap2", "gap3"};
    if (ret == 0)
        ret = process_cpu_se_stage2(inimage, in4, outimage, cache);
...
int RealCUGAN::process_cpu_se_very_rough(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    std::vector<std::string> in0 = {};
    std::vector<std::string> out0 = {"gap0", "gap1", "gap2", "gap3"};
    ret = process_cpu_se_very_rough_stage0(inimage, in0, out0, cache);

    std::vector<std::string> gap0 = {"gap0", "gap1", "gap2", "gap3"};
    if (ret == 0)
        ret = process_cpu_se_very_rough_sync_gap(inimage, gap0, cache);
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (tta_mode)
            {
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    for (size_t i = 0; ret == 0 && i < names.size(); i++)
                    {
                        ncnn::VkMat feat;
                        ...
                        if (feat.empty())
                            ...
                            ret = ex.input(names[i].c_str(), feat);
                    }

                    for (size_t i = 0; ret == 0 && i < outnames.size(); i++)
                    {
                        ncnn::VkMat feat;
                        ret = ex.extract(outnames[i].c_str(), feat, cmd);
                        if (ret == 0 && feat.empty())
                            ret = ERROR_EMPTY_MAT;
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (tta_mode)
            {
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    {
                        ncnn::Mat feat;
                        ret = ex.extract(outnames[i].c_str(), feat);
                        if (ret == 0 && feat.empty())
                            ret = ERROR_EMPTY_MAT;
