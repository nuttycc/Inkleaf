...
constexpr int ERROR_INPUT_TOO_SMALL = -101;
...
#endif
{
    ...
    if (vkdev)
    {
...

        {
            ...
            if (spirv.empty() ||
                realesrgan_preproc->create(spirv.data(), spirv.size() * 4, specializations) != 0)
                return -4;
        }

        {
            ...
            if (spirv.empty() ||
                realesrgan_postproc->create(spirv.data(), spirv.size() * 4, specializations) != 0)
                return -4;
        }
    }

    // bicubic interpolation for alpha channel
    {
...
int RealESRGAN::process(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    if (inimage.w <= prepadding || inimage.h <= prepadding)
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (tta_mode)
            {
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    int ret = ex.input("data", in_tile_gpu[ti]);
                    if (ret == 0)
                        ret = ex.extract("output", out_tile_gpu[ti], cmd);
            ...
            else
            {
                ...
                ncnn::VkMat out_tile_gpu;
                {
                    ...
                    int ret = ex.input("data", in_tile_gpu);
                    if (ret == 0)
                        ret = ex.extract("output", out_tile_gpu, cmd);
...
int RealESRGAN::process_cpu(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    for (int yi = 0; yi < ytiles; yi++)
    {
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            if (in.empty() || (channels == 4 && in_nopad.empty()))
            ...
            if (tta_mode)
            {
                ...
                // border padding
                {
                    ...
                    ncnn::copy_make_border(in_tile[0], in_tile_padded, pad_top, pad_bottom, pad_left, pad_right, ncnn::BORDER_REFLECT, 0.f, net.opt);
                ...
                for (int ti = 0; ti < 8; ti++)
                {
                    ...
                    int ret = ex.input("data", in_tile[ti]);
                    if (ret == 0)
                        ret = ex.extract("output", out_tile[ti]);
                    if (ret != 0)
                        return ret;
            ...
            else
            {
                ...
                // border padding
                {
                    ...
                    ncnn::Mat in_tile_padded;
                    ncnn::copy_make_border(in_tile, in_tile_padded, pad_top, pad_bottom, pad_left, pad_right, ncnn::BORDER_REFLECT, 0.f, net.opt);
                ...
                ncnn::Mat out_tile;
                {
                    ...
                    int ret = ex.input("data", in_tile);
                    if (ret == 0)
                        ret = ex.extract("output", out_tile);
                    if (ret != 0)
                        return ret;
                    if (out_tile.empty())
                        return ERROR_EMPTY_MAT;
                ...
                // postproc and merge alpha
                {
                    ...
                    if (out_tile.w < out.w + crop * 2 || out_tile.h < out.h + crop * 2)
                        return ERROR_EMPTY_MAT;
                    for (int q = 0; q < 3; q++)
                    {
                        float* outptr = out.channel(q);
