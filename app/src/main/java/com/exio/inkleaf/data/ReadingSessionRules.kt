 ...
 *
 * No Android, Room, or UI dependencies. Reader/platform code maps lifecycle
 * signals into these inputs; the repository applies the resulting decisions
 * inside transactions.
 *
...
object ReadingSessionRules {
    ...
    /**
     * A session enters permanent history when either threshold is met:
     * 30s of effective reading, or the end/checkpoint position differs from start.
     */
    ...
    /**
     * Adds [deltaMillis] of interactive foreground time.
     * Negative deltas are rejected; zero is a no-op.
     */
    ...
    /**
     * Interactive segment length from [segmentStartedAt] to [nowMillis].
     * Null segment start means timing was already paused — contribute 0.
     */
    ...
    /**
     * Decide whether opening [incomingFileKey] should resume [existing]
    ...
    fun decideResume(
        ...
        val sameSource =
            existing.checkpointPosition.sourceRevision == incomingSourceRevision
        ...
        return when {
            ...
            !sameComic -> ResumeDecision.SettleThenStart(
                sessionId = existing.id,
                reason = ReadingSessionEndReason.SWITCHED_COMIC,
            )
            !sameSource -> ResumeDecision.SettleThenStart(
                sessionId = existing.id,
                reason = ReadingSessionEndReason.SOURCE_CHANGED,
            )
            else -> ResumeDecision.SettleThenStart(
                sessionId = existing.id,
    ...
    /**
     * After process restart, an orphan ACTIVE/PAUSED row is either still
     * resumable or settled at its last checkpoint.
     */
