...
class LibraryScanner(context: Context) {
     ...
     */
    suspend fun scanFolder(treeUri: Uri): List<ScannedFile> =
                    ...
                    val providerStillLoading =
                        visitChildren(treeUri, directory.docId) { child ->
                            ...
                            if (child.isDirectory) {
                                ...
                                if (visitedDirectories.add(visitKey)) {
                                    metrics =
                                        metrics.copy(directoryCount = metrics.directoryCount + 1)
                                    evaluateLimit(metrics, confirmationThresholds)?.let { decision
                                        ->
                                        stopReason = decision.first
                                        stopLimit = decision.second
                                ...
                                val fileKey =
                                    ComicIdentity.safDocumentKey(treeUri.authority, child.docId)
    ...
    companion object {
        ...
        val SOFT_SCAN_THRESHOLDS =
            ScanThresholds(
                pdfCount = 500,
                directoryCount = 1_000,
                entryCount = 10_000,
            )
        val HARD_SCAN_THRESHOLDS =
            ScanThresholds(
                pdfCount = 2_000,
        ...
        internal fun sortPdfs(files: List<ScannedFile>): List<ScannedFile> =
            files.sortedWith { first, second ->
                ChapterSort.compareNatural(first.displayName, second.displayName).takeIf { it != 0 }
                    ?: ChapterSort.compareNatural(first.relativePath, second.relativePath).takeIf {
                        it != 0
                    }
        ...
        private fun reachedLimit(metrics: ScanMetrics, limits: ScanThresholds): ScanLimit? =
            when {
        ...
        private fun exceededHardLimit(metrics: ScanMetrics): ScanLimit? =
            when {
                metrics.pdfCount > HARD_SCAN_THRESHOLDS.pdfCount -> ScanLimit.PDFS
                metrics.directoryCount > HARD_SCAN_THRESHOLDS.directoryCount ->
                    ScanLimit.DIRECTORIES
