...
val OcrModelVariant.totalBytes: Long
...
internal val OCR_MODEL_FILES: List<OcrModelFileSpec>
    get() = OcrModelVariant.SMALL.files
internal val OCR_MODEL_TOTAL_BYTES: Long
    get() = OcrModelVariant.SMALL.totalBytes
...
internal val OCR_MODEL_SOURCES =
    listOf(
        OcrModelSource("HuggingFace") { repo, file ->
            "https://huggingface.co/PaddlePaddle/$repo/resolve/main/$file"
        },
        OcrModelSource("ModelScope") { repo, file ->
            "https://modelscope.cn/models/PaddlePaddle/$repo/resolve/master/$file"
        },
        OcrModelSource("hf-mirror") { repo, file ->
...
internal fun isOcrModelReady(
    filesDir: File,
    variant: OcrModelVariant = OcrModelVariant.SMALL,
): Boolean {
    val dir = ocrModelDir(filesDir, variant)
    if (!File(dir, ".version").let { it.exists() && it.readText().trim() == variant.version })
        return false
