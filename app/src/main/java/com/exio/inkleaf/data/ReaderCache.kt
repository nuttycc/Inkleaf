 ...
 *
 * 两类缓存的失效是联动的——zip 副本因原文件变化而重建时，
 * 这本书的缩略图也一并作废（它们派生自旧内容）。把路径和清理
 * 集中在一处，调用方不可能只清一半。
 *
 * 都放 cacheDir：系统存储紧张时可整体回收，回收后行为退化为
 * "重新复制/重新解码"，不丢任何用户数据。
 */
object ReaderCache {
    ...
    fun booksDir(context: Context): File =
        File(context.cacheDir, BOOKS_DIR).apply { mkdirs() }

    /**
     * 某本书的 zip 副本文件。文件名内嵌原文件的"大小+修改时间"作为
     * 校验键：原文件被替换后键变化，旧副本自然失配（由 wipeBook 清走），
     * 不需要额外的元数据文件
     */
    ...
    /**
     * 副本总量超预算（用户设置的档位或自动推荐值）时按 lastModified（每次打开会刷新，
     * 即"最近阅读时间"）从旧到新淘汰，连带删掉对应书的缩略图。
     * keep 是当前正打开的副本，任何情况下不淘汰它；设置页触发的清理
     * 没有"当前书"概念，传 null——即使误删了后台阅读页正持有的副本
     * 也安全：已打开的文件句柄在 unlink 后仍然有效，下次打开重新复制。
     */
