 ...
 *
 * Each session belongs to the local calendar day of its start time in the zone captured at session
 * start. Crossing midnight does not split a session. Relative labels ("今天" / "昨天") use the device's
 * current date and refresh without rewriting stored rows.
 *
 * Spec: #13 timeline IA, #18 insertSeparators + stable date keys.
 */
object HistoryDateGrouping {
    /**
     * Local calendar date for a session start, using the session's own zone. Device travel later
     * must not move the grouping day.
     */
    ...
    /**
     * Human label for a date header. "今天"/"昨天" relative to [today]; older dates use a localized
     * medium date.
     */
    fun labelFor(
    ...
    /**
     * Inserts a date header before the first item of each local day. Items must already be ordered
     * newest-first (startedAt DESC, id DESC).
     *
...
/**
 * Flattened timeline rows after date separators are inserted. Stable Compose keys:
 * `date:<local-date>` and `session:<uuid>`.
 */
sealed interface HistoryTimelineEntry {
    ...
    data class Session(val sessionId: String) : HistoryTimelineEntry {
