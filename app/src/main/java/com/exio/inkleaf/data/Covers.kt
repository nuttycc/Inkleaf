...
object Covers {
    ...
    /** 把一页漫画的原始字节缩小保存为封面 JPEG，返回封面文件；失败返回 null （封面只是锦上添花，任何失败都不应影响阅读功能）。 */
    suspend fun createCoverFile(context: Context, comicId: Long, pageBytes: ByteArray): File? =
        withContext(Dispatchers.IO) {
            runCatching {
                    val bitmap = decodeSampled(pageBytes, TARGET_WIDTH) ?: return@runCatching null
    ...
    suspend fun createCoverFile(context: Context, comicId: Long, imageFile: File): File? =
        withContext(Dispatchers.IO) {
            runCatching {
                    val bitmap = decodeSampled(imageFile, TARGET_WIDTH) ?: return@runCatching null
                    saveCoverBitmap(context, comicId, bitmap)
                }
                .getOrNull()
        }
     ...
     * 两步降采样解码（封面与胶片缩略图共用）。
     *
     * 为什么不直接解码原图再缩小？一张 2000x3000 的页解码后占 2000*3000*4 ≈ 23MB 内存，只为了生成一张缩略图太浪费，还可能 OOM。 标准做法分两步：
     * 1. inJustDecodeBounds = true 只读出图片尺寸，不分配像素内存
     * 2. 按目标宽度算 inSampleSize（只能取 2 的幂，每翻倍宽高各减半）， 解码时直接按 1/N 尺寸读出，内存占用降为 1/N²
     *
     * config 传 RGB_565 时每像素 2 字节（无透明通道），缩略图场景 画质无损感、内存再减半；null = 用系统默认（ARGB_8888）。
     */
