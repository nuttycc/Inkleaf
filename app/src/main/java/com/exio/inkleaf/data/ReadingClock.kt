 ...
 *
 * Session windows, checkpoints, and date grouping must not call
 * System.currentTimeMillis() directly — tests advance a fake clock instead
 * of sleeping through the 10-minute interruption window.
 *
 * [zoneId] is read once when a session starts and stored on the row.
 * Later calls must not rewrite an existing session's zone.
 */
