 ...
 * 跨表投影：目录信息 + 该目录下的漫画数。
 *
 * @Embedded 把 SELECT 里 library_folders 的列"摊平"进 folder 字段， 额外的 comicCount 列对应同名属性。
 */
...
interface LibraryFolderDao {
    /** 子查询统计每个目录的漫画数。Room 会同时追踪两张表： comics 表一变（扫描入库），这个 Flow 也会推送新列表 */
    @Query(
    ...
    @Query("DELETE FROM library_folders WHERE id = :id") suspend fun deleteById(id: Long)
}
