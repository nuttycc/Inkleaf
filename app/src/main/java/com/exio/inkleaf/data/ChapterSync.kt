...
object ChapterSync {
    ...
    fun computeDiff(
        ...
        scanned.forEachIndexed { index, pdf ->
            ...
            if (cur == null) {
                ...
                val contentChanged = metadataChanged(cur.size, pdf.size) ||
        ...
        val toMarkMissing = if (completeScan) {
            existing
    ...
    private fun buildChapterTitles(
        ...
        return groupScannedFilesByDisplayName(scanned).values.flatMap { group ->
            if (group.size == 1) {
    ...
    private fun duplicatePathTitles(
        ...
        val suffixCounts = (2..maxDepth).associateWith { length ->
            duplicates.groupingBy { file ->
                segmentsByKey.getValue(file.fileKey).takeLast(length).joinToString("/").lowercase()
            }.eachCount()
        }

        val candidates = duplicates.associate { file ->
            val segments = segmentsByKey.getValue(file.fileKey)
            val uniqueSuffix = (2..maxDepth).firstNotNullOfOrNull { length ->
        ...
        return buildMap {
            repeatedCandidates.values.forEach { entries ->
                if (entries.size == 1) {
                    ...
                    entries.sortedBy { it.key }.forEachIndexed { index, entry ->
