...
object AlbumProgress {
    ...
    fun remap(
        ...
        val oldIndex =
            lastReadPageId?.let(oldPageIds::indexOf)?.takeIf { it >= 0 }
...
class AlbumRepository(context: Context) {
    ...
    suspend fun loadAlbum(comicId: Long): AlbumSnapshot {
        val comic =
            comicDao.getById(comicId)?.takeIf { it.sourceType == BookSourceType.CREATED_ALBUM }
    ...
    suspend fun saveAlbum(
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val oldCoverCanBeReused =
                    existingComic?.let { comic ->
                        comic.coverPageId == selectedCoverId &&
                            comic.coverPath?.let(::File)?.isFile == true
                    } == true
                val coverPath =
                    if (oldCoverCanBeReused) {
            ...
            albums.forEach { comic ->
                ...
                val referenced =
                    pages
                        .mapNotNull { page ->
                            runCatching {
                                    resolveAlbumPageFile(appContext.filesDir, page.relativePath)
                                        .canonicalPath
                                }
                                .getOrNull()
                        }
                ...
                File(appContext.filesDir, "covers")
                    .listFiles()
                    .orEmpty()
        ...
        drafts.forEach { draft ->
            ...
            val file =
                if (draft.isStaged) {
                    ...
                    val old = oldById[draft.id] ?: throw IllegalArgumentException("页面不属于当前图册")
    ...
    private fun deleteRemovedPageFiles(
        ...
        oldPages
            .filter { it.relativePath !in retained }
            .forEach { page ->
                runCatching {
                    resolveAlbumPageFile(appContext.filesDir, page.relativePath).delete()
                }
            }
    }

    private fun AlbumPageEntity.toDraft(): AlbumPageDraft =
        AlbumPageDraft(
            id = id,
    ...
    private fun preservedExtension(extension: String, mimeType: String?): String? =
        when {
    ...
    private inline fun <T> catchImportFailure(block: () -> T): T? =
        try {
            block()
    ...
    companion object {
        ...
        private val IMAGE_EXTENSIONS =
            PRESERVED_EXTENSIONS + setOf("heic", "heif", "avif", "bmp", "tif", "tiff")
