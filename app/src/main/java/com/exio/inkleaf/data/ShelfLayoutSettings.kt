...
/**
 * DataStore 实例必须全局唯一：用顶层属性委托挂在 Context 上是官方惯用法。
 * 同名 DataStore 创建两次会直接抛异常——这是 DataStore 的第一坑。
 */
