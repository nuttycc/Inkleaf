 ...
 *
 * Platform code (ReaderViewModel + ProcessLifecycleOwner) translates Android
 * signals into [ReadingSessionEvent] values. Configuration changes must never
 * emit events — brief Activity teardown is continuous reading.
 *
 ...
 *
 * [interactiveForeground] defaults to false. If the process is already in the
 * foreground when the Reader attaches, the platform MUST still emit an explicit
 * EnteredInteractiveForeground; otherwise a session never starts.
 *
 * [ReadingSessionEvent.OpenComic] only identifies the comic. Source revision is
 * compared later on ReaderReady once the opened volume is known.
 *
 * After [ReadingSessionEvent.ProcessRestored], the slot stays PAUSED until the
 * matching Reader satisfies Ready + PageVisible + Foreground again.
 *
...
class ReadingSessionStateMachine(
    ...
    /**
     * Wall-clock start of the current interactive foreground segment.
     * Null while paused, completed, or before the session has started timing.
     */
    ...
    /**
     * True after at least one PageVisible for the current Ready book.
     * Ready alone must not set this — loading success ≠ page actually shown.
     */
    var pageVisible: Boolean = false
        private set

    /**
     * True while ProcessLifecycleOwner reports interactive foreground.
     * Defaults false so a session never starts without an explicit enter event.
     */
    ...
    private fun settleForeignOrActive(
        ...
        val activelyTiming = existing.status == ReadingSessionStatus.ACTIVE &&
            interactiveSegmentStartedAt != null
        ...
        return settle(
            current = existing,
            reason = reason,
            settlementInstant = settlementInstantFor(
                existing,
                now,
        ...
        val effects = if (current != null) {
            settle(
                ...
                settlementInstant = settlementInstantFor(
                    current,
                    now,
                    wasActivelyTiming = current.status == ReadingSessionStatus.ACTIVE &&
                        interactiveSegmentStartedAt != null,
                ),
            )
        } else {
    ...
    private fun onProcessRestored(
        ...
        val normalized = if (existing.status == ReadingSessionStatus.ACTIVE) {
        ...
        return when (val decision = ReadingSessionRules.decideProcessRecovery(normalized, now)) {
            ...
            is ProcessRecoveryDecision.Settle -> settle(
                current = normalized,
    ...
    private fun tryStart(): List<ReadingSessionEffect> {
        ...
        if (existing != null) {
            ...
            val sameSource =
                existing.checkpointPosition.sourceRevision == position.sourceRevision
            val withinWindow =
                ReadingSessionRules.isWithinInterruptionWindow(existing.lastCheckpointAt, now)
            if (!sameSource || !withinWindow) {
                val reason = when {
    ...
    private fun pauseActive(
        ...
        val delta = if (accumulateSegment) {
        ...
        val permanent = current.isPermanent ||
    ...
    private fun settle(
        ...
        val delta = if (settlementInstant.includeActiveSegment) {
        ...
        val active = ReadingSessionRules.accumulateActiveReading(current.activeReadingMillis, delta)
        val permanent = ReadingSessionRules.qualifiesAsPermanent(
            activeReadingMillis = active,
        ...
        return if (permanent) {
            ...
            val checkpointPosition = if (settlementInstant.includeActiveSegment) {
                position
            } else {
    ...
    /**
     * Paused / recovered sessions end at their stored checkpoint time.
     * Actively timing sessions end at [now] and fold in the open segment.
     */
...
/**
 * Domain events. Configuration changes intentionally have no event —
 * emitting nothing keeps the session continuous across Activity recreation.
 */
sealed interface ReadingSessionEvent {
    /**
     * Reader is opening a comic. Source revision is unknown until the volume
     * opens — only [comic] is available here. Settles a different fileKey as
     * SWITCHED_COMIC; same-key resume/timeout/source checks wait for Ready.
     */
    data class OpenComic(
        val comic: ReadingSessionComicRef,
    ) : ReadingSessionEvent

    /**
     * Comic content reached Ready. Carries the opened volume's start position
     * (including sourceRevision) so deferred source checks can run.
     */
    ...
    /**
     * App left interactive foreground (background, lock screen, multi-window
     * unfocus, covering Activity). Pauses timing; does not end the session.
     */
