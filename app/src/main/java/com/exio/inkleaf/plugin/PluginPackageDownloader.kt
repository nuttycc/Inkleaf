...
class PluginPackageDownloader(
    ...
    suspend fun download(
        ...
        require(url.startsWith("https://") || url.startsWith("http://")) { "Only HTTP(S) plugin URLs are supported" }
        ...
        if (completed.isFile && completedSizeMatches && completedHashMatches) {
            onProgress(PluginDownloadProgress(completed.length(), completed.length(), resumed = false))
        ...
        response.use { result ->
            if (!result.isSuccessful && result.code != 206) throw IOException("Plugin download failed with HTTP ${result.code}")
            val resumed = existingBytes > 0L && result.code == 206
            val startBytes = if (resumed) existingBytes else 0L
            if (!resumed && partial.exists() && !partial.delete()) throw IOException("Unable to restart partial plugin download")
        ...
        source.expectedSizeBytes?.let { expected ->
            if (partial.length() != expected) throw IOException("Plugin download size does not match metadata")
        }
        source.expectedSha256?.let { expected ->
            if (!expected.equals(sha256(partial), ignoreCase = true)) throw IOException("Plugin SHA-256 does not match metadata")
        }
        if (completed.exists() && !completed.delete()) throw IOException("Unable to replace completed plugin download")
        if (!partial.renameTo(completed)) throw IOException("Unable to finalize plugin download")
        completed
    }
