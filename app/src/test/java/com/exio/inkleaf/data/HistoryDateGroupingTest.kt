...
class HistoryDateGroupingTest {
    ...
    fun `session local date uses session zone not device travel`() {
        ...
        val startedAt = java.time.ZonedDateTime
            .of(2024, 6, 2, 2, 0, 0, 0, tokyo)
            .toInstant()
            .toEpochMilli()
    ...
    fun `relative labels use device today`() {
        ...
        val older = HistoryDateGrouping.labelFor(
    ...
    fun `separators insert once per local day on newest-first list`() {
        ...
        val timeline = HistoryDateGrouping.withDateSeparators(
            items = rows,
    ...
    fun `cross-midnight session stays on start day`() {
        ...
        val start = java.time.ZonedDateTime
            .of(2024, 3, 10, 23, 50, 0, 0, ZoneId.of(zone))
            .toInstant()
            .toEpochMilli()
