...
class PluginContentModelsTest {
    ...
    fun `descriptors without feeds remain compatible`() {
        val decoded = PluginContentCodec.describe(
                ...
                feeds = listOf(
                    PluginFeedDescriptor(
                        ...
                        filters = listOf(
                            ...
                            PluginFilterDescriptor("future", "Future", type = "futureFilter"),
                        ),
                    )
                )
            )
    ...
    fun `feed and filter identifiers must be unique within their scope`() {
        val duplicateFeeds = PluginDescribeResponse(
            feeds = listOf(
                PluginFeedDescriptor("rank", "Ranking"),
                PluginFeedDescriptor("rank", "Duplicate"),
            )
        )
        val duplicateOptions = PluginDescribeResponse(
            feeds = listOf(
                PluginFeedDescriptor(
                    "rank",
                    "Ranking",
        ...
        assertTrue(
            runCatching {
                PluginContentCodec.describe(PluginContentCodec.json.encodeToJsonElement(duplicateFeeds))
            }.exceptionOrNull() is PluginContentValidationException
        )
        assertTrue(
            runCatching {
                PluginContentCodec.describe(PluginContentCodec.json.encodeToJsonElement(duplicateOptions))
            }.exceptionOrNull() is PluginContentValidationException
        )
    }
    ...
    fun `search item source IDs must be unique`() {
        val duplicateItems = PluginSearchPage(
            items = listOf(
    ...
    fun `browse requests encode feed cursor limit and filters`() {
        val encoded = PluginContentCodec.encode(
            PluginBrowseRequest(
                feedId = "rank",
                cursor = "21",
                limit = 21,
